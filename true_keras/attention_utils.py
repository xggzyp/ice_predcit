...
def attention_decoder_fn_train(encoder_state,
    ...
    def decoder_fn(time, cell_state, cell_input, cell_output, context_state):
        ...
        with K.name_scope(
            ...
            next_input = Concatenate([cell_input, attention], 1)
...
def prepare_attention(attention_states,  num_units
    ...
    with tf.variable_scope("attention_keys", reuse=reuse) as scope:
        attention_keys = Dense(num_units, bias_initializer='zeros')(attention_states)
...
def _create_attention_construct_fn(name, num_units, attention_score_fn, reuse=None):
    ...
    def construct_fn(attention_query, attention_keys, attention_values):
        with tf.variable_scope(name, reuse=reuse) as scope:
            ...
            #attention_query=tf.reshape(attention_query,(-1,128))
            attention_query=Lambda(resh)(attention_query)
            concat_input = Concatenate(axis=1)([attention_query, context])
            attention = Dense(num_units, bias_initializer='zeros')(concat_input)
            return attention
