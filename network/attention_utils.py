...
from keras.layers import Dense,Lambda,Concatenate, Reshape,Multiply
...
def attention_decoder_fn_train(encoder_state,
    ...
    """
    with K.name_scope(name, "attention_decoder_fn_train", [
    ...
    def decoder_fn(time, cell_state, cell_input, cell_output, context_state):
        ...
        with K.name_scope(
            ...
            next_input = Concatenate(1)([cell_input, attention])
...
def attention_decoder_fn_inference(output_fn,
    ...
    Args:
      ...
      ```
        K.variable_scope("decoder") as varscope
          output_fn = lambda x: K.contrib.layers.linear(x, num_decoder_symbols,
                                              scope=varscope)
      ...
      dtype: (default: `K.float32`) The default data type to use when
    ...
    with tf.name_scope(name, "attention_decoder_fn_inference", [
        ...
        if batch_size is None:
            batch_size = Reshape(encoder_info)[0]
...
## Helper functions ##
def prepare_attention(attention_states,  num_units
                      ,attention_option="luong",reuse=None):

    # Prepare attention keys / values from attention_states
    with tf.variable_scope("attention_keys", reuse=reuse) as scope:
        attention_keys = Dense(num_units)(attention_states)
...
def _init_attention(encoder_state):
    ...
    if isinstance(top_state, tf.contrib.rnn.LSTMStateTuple):
        attn = K.zeros_like(top_state.h)
    else:
        attn = K.zeros_like(top_state)

    return attn


def _create_attention_construct_fn(name, num_units, attention_score_fn, reuse=None):
    ...
    def construct_fn(attention_query, attention_keys, attention_values):
        with tf.variable_scope(name, reuse=reuse) as scope:
            context = attention_score_fn(attention_query, attention_keys,
                                         attention_values)
            # print("cvdfuahfg")
            # print(attention_query.shape,context.shape)
            # attention_query=K.reshape(attention_query,(-1,128))
            concat_input = Concatenate(axis=1)([attention_query, context])
            attention = Dense(num_units)(concat_input)
            return attention
...
def _attn_add_fun(v, keys, query):
    return K.sum(v * K.tanh(keys + query), [2],keepdims=False)


@function.Defun(func_name="attn_mul_fun", noinline=True)
def _attn_mul_fun(keys, query):
    return K.sum(keys * query, [2],keepdims=False)
...
def _create_attention_score_fn(name,
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        def attention_score_fn(query, keys, values):
            ...
            def resha(x):
                return tf.reshape(x, [-1, 1, num_units])
            if attention_option == "bahdanau":
                # transform query
                query = Multiply(query, query_w)

                # reshape query: [batch_size, 1, num_units]
                query = Lambda(resha)(query)
                ...
                # reshape query: [batch_size, 1, num_units]
                #query = Lambda(resha)(query)
                query=K.reshape(query,[-1, 1, num_units])

                # attn_fun
                scores = _attn_mul_fun(keys, query)
                # scores=Lambda(_attn_mul_fun)(keys, query)
            else:
            ...
            # alignments = Lambda(expand_dim)(alignments)
            alignments=K.expand_dims(alignments,axis=2)
            context_vector = K.sum(alignments * values, [1],keepdims=False)
            # print("fhdaui{}".format(context_vector))
