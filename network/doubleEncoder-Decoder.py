...
def gen_encoder(hparmas,inputs):
    encoder1 = LSTM(hparmas.gen_rnn_size, return_state=True, return_sequences=True, dropout=hparmas.gen_vd_keep_prob, recurrent_dropout=hparmas.gen_vd_keep_prob)(
        inputs)
    encoder = LSTM(hparmas.gen_rnn_size, return_state=True, return_sequences=True, dropout=hparmas.gen_vd_keep_prob, recurrent_dropout=hparmas.gen_vd_keep_prob)(encoder1)
    encoder_outputs, state_h, state_c = encoder
    encoder_states = [state_h, state_c]
    return encoder_outputs,encoder_states

def gen_decoder(hparmas,parser,encoder_outputs,states,n_output):
    input=tf.zeros(shape=(None,1))

    def cell_lstm(input_l, state_l):
        decoder_lstm1 = LSTM(hparmas.gen_rnn_size, return_sequences=True, return_state=True, dropout=0.5, recurrent_dropout=0.5)(
            input_l,
            initial_state=state_l)
        decoder_outputs, state_h, state_c = LSTM(hparmas.gen_rnn_size, return_state=True, dropout=0.5,
                                                 recurrent_dropout=0.5)(decoder_lstm1)
        state = [state_h, state_c]
        return decoder_outputs, state
    ...
    # initial_state[0]=Lambda(reshap)(initial_state[0])
    (attention_keys, attention_values) = Lambda(pre_attein)(encoder_outputs)
    attention_score_fn = attention_utils._create_attention_score_fn("attention_keys", hparmas.gen_rnn_size, "luong")
    # Attention construction function
    attention_construct_fn = attention_utils._create_attention_construct_fn("attention_score",
                                                                            hparmas.gen_rnn_size, attention_score_fn)
    attention_option = parser.attention_option
...
def dis_encoder(hparmas,inputs):
    encoder1 = LSTM(hparmas.gen_rnn_size, return_state=True, return_sequences=True, dropout=hparmas.gen_vd_keep_prob,
                    recurrent_dropout=hparmas.gen_vd_keep_prob)(
        inputs)
    encoder = LSTM(hparmas.gen_rnn_size, return_state=True, return_sequences=True, dropout=hparmas.gen_vd_keep_prob,
                   recurrent_dropout=hparmas.gen_vd_keep_prob)(encoder1)
    encoder_outputs, state_h, state_c = encoder
    encoder_states = [state_h, state_c]
    return encoder_outputs, encoder_states
    return
...
def define_models(n_input, n_output, n_units, parser, n_steps_out,hparmas):
    # define training encoder
    inputs = Input(shape=(None, n_input))
    encoder_outputs, encoder_states=gen_encoder(hparmas,inputs)
    sequence,_=gen_decoder(hparmas,parser,encoder_outputs,encoder_states,n_output)
    # define training decoder
    # print("woshostate_h{}\nwoshiostate_c{}".format(state_h, state_c))
    decoder_inputs = Input(shape=(None, n_output))
    dis_encoder_outputs,dis_encoder_states=dis_encoder(hparmas,inputs)
    fake_prediction=dis_decoder(hparmas,parser,dis_encoder_outputs,dis_encoder_states,sequence,n_output)
    real_prediction=dis_decoder(hparmas,parser,dis_encoder_outputs,dis_encoder_states,decoder_inputs,n_output)
